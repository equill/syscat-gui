         ...
         (default-clwho-layout
           ...
           (cl-who:with-html-output-to-string
             ...
             (:form
               ...
               :method "post"
               ;; Source resourcetype
               (:div :id "source-resource"
                (:h3 "Source resource: ")
                (:div (format content-string "~A of resourcetype ~A"
                              (tbnl:get-parameter "source")
                              source-resourcetype)))
               ;; Relationship types
               (:div :id "available-rels"
                (:h3 (format content-string "Available relationships from ~A to ~A: "
               ...
               ;; Submit button
               (:input :id "linkbutton" :type "submit" :value "Create relationship")))))))
                 ...
                 (let ((message (format nil "Failed to add relationship ~A from ~A to '~A'. ~A: ~A"
                   ...
                   (with-output-to-string
                     ...
                     (default-clwho-layout
                       ...
                       :stylesheets '("display")
                       :javascripts '("display")
                       :content (display-default outstr (list :p message)))))
                 ;; Happy path: no errors
